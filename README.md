...
- **Concurrency caps are usually below five.** `createReviewSandbox` backs off on `ConcurrencyLimit` / `NoCapacity` and reclaims our own orphans on the first retry. The floor shows five doors; `FORKLIFT_BAY_CONCURRENCY` (default 2) is how many run at once.
